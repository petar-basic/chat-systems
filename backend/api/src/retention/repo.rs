...
impl RetentionRepo {
    ...
    pub async fn get(&self, workspace_id: Uuid) -> sqlx::Result<Option<RetentionPolicy>> {
        sqlx::query_as!(
            RetentionPolicy,
            "SELECT workspace_id, message_days, file_days, audit_days, notification_days, updated_by, updated_at FROM retention_policies WHERE workspace_id = $1",
            workspace_id
        )
    ...
    pub async fn upsert(
        ...
        sqlx::query_as!(
            ...
            ON CONFLICT (workspace_id) DO UPDATE SET
                ...
                updated_at = NOW()
            RETURNING workspace_id, message_days, file_days, audit_days, notification_days, updated_by, updated_at
            ",
            workspace_id,
            req.message_days,
            req.file_days,
    ...
    pub async fn workspaces_with_policies(&self) -> sqlx::Result<Vec<RetentionPolicy>> {
        sqlx::query_as!(RetentionPolicy, "SELECT workspace_id, message_days, file_days, audit_days, notification_days, updated_by, updated_at FROM retention_policies")
            .fetch_all(&self.pool)
            .await
    }
    ...
    pub async fn delete_file_rows(&self, ids: &[Uuid]) -> sqlx::Result<u64> {
        ...
        let result = sqlx::query!("DELETE FROM files WHERE id = ANY($1)", ids)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected())
    }
    ...
    pub async fn count_expired_tokens(&self) -> sqlx::Result<(i64, i64, i64, i64)> {
        let reset = sqlx::query_scalar!(
            r#"SELECT COUNT(*) AS "count!" FROM password_reset_tokens WHERE expires_at < NOW()"#
        )
        .fetch_one(&self.pool)
        .await?;
        let refresh = sqlx::query_scalar!(
            r#"SELECT COUNT(*) AS "count!" FROM refresh_tokens WHERE expires_at < NOW()"#
        )
        .fetch_one(&self.pool)
        .await?;
        let invites = sqlx::query_scalar!(
            r#"SELECT COUNT(*) AS "count!" FROM workspace_invites WHERE expires_at IS NOT NULL AND expires_at < NOW()"#
        )
        .fetch_one(&self.pool)
        .await?;
        let executions = sqlx::query_scalar!(
            r#"SELECT COUNT(*) AS "count!" FROM hook_executions WHERE executed_at < NOW() - INTERVAL '30 days'"#
        )
        .fetch_one(&self.pool)
        .await?;
        Ok((reset, refresh, invites, executions))
