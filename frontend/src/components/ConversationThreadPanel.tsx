...
function ThreadReply({ message }: { message: ConversationMessage }) {
  const { getUser } = useUserCache();
  const sender = getUser(message.user_id);
  const displayName = displayNameOf(sender?.display_name);

  return (
    <div className="flex items-start gap-2.5 py-1.5" data-qa="dm-thread-reply">
      <Avatar
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-slate-700/30">
        ...
        <div className="mt-2 text-xs text-slate-400" data-qa="dm-thread-count">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2">
        {isLoading ? (
          ...
          <div className="text-center py-8 text-slate-400 text-sm">No replies yet.</div>
        ) : (
          replies.map((reply) => <ThreadReply key={reply.id} message={reply} />)
        )}
        <div ref={endRef} />
      </div>
