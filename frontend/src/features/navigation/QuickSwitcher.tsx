...
export function QuickSwitcher({
  ...
  return (
    <div
      className="fixed inset-0 bg-overlay/50 flex items-start justify-center pt-[15vh] z-50"
      onClick={onClose}
    >
      <div
            ...
            items.map((item, i) => (
              <button
                ...
                className={`w-full px-4 py-2 flex items-center gap-3 text-left transition ${
                  i === active ? 'bg-purple-600/20' : 'hover:bg-raised/40'
                }`}
              >
                <span className="shrink-0">{item.icon}</span>
                <span className="flex-1 text-sm text-fg-soft truncate">{item.label}</span>
                {item.sub && <span className="text-xs text-muted shrink-0">{item.sub}</span>}
              </button>
            ))
          )}
        </div>
